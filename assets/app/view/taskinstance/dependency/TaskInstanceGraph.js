Ext.define('App.view.taskinstance.dependency.TaskInstanceGraph',{
    ...
    convertToNode: function(task){
        ...
        t.children = _.map(task.predecessorInstances,function(p){
            return {
    ...
    convertTaskToNode: function(task){
        ...
        node.children = _.map(task.predecessorInstances,function(p){
            return {
                name : p.task.name,
                excluded: p.excluded,
                instanceId  : p.id,
    ...
    init  : function(task){
        ...
        function update(source) {
            ...
            var nodeEnter = node.enter().append("g")
                ...
                .on('contextmenu',function(d){
                    ...
                    me.lookupController()
                        .showTaskMenu([d3.event.x,d3.event.y],d.instanceId,nodes,svg);
                });
        ...
        function click(d) {
            ...
            if(_.isEmpty(d.children) && _.isEmpty(d._children)){
                AsyncTaskInstance.find(d.instanceId).success(function(data){
                    if(! _.isEmpty(data) ){
                        d.children = me.convertTaskToNode(data[0]).children;
